...
// 2. Add cliMappings menu item
const mcpServerMenuItem = `<button onClick={() => setActiveMenu('mcpServer')} className={\`w-full text-left px-3 py-1.5 cursor-pointer flex items-center gap-2 transition-all \${activeMenu === 'mcpServer' ? 'bg-cyan-950/40 border-r-[3px] border-cyan-400 text-cyan-300' : 'text-cyan-600/60 hover:text-cyan-500/80'}\`}>
                <Server className={\`w-3 h-3 \${activeMenu === 'mcpServer' ? 'text-cyan-400' : 'text-cyan-700'}\`} />
                <span>{t.mcpServer}</span>
              </button>`;
const cliMappingsMenuItem = `<button onClick={() => setActiveMenu('cliMappings')} className={\`w-full text-left px-3 py-1.5 cursor-pointer flex items-center gap-2 transition-all \${activeMenu === 'cliMappings' ? 'bg-cyan-950/40 border-r-[3px] border-cyan-400 text-cyan-300' : 'text-cyan-600/60 hover:text-cyan-500/80'}\`}>
                <Terminal className={\`w-3 h-3 \${activeMenu === 'cliMappings' ? 'text-cyan-400' : 'text-cyan-700'}\`} />
                <span>CLI Adaptors</span>
              </button>`;

content = content.replace(mcpServerMenuItem, mcpServerMenuItem + '\n              ' + cliMappingsMenuItem);

// 3. Add cliMappings tab content
const mcpServerTabEnd = `          * Note: Embedded Jarvis Core will spawn authentic Node child_processes and handle stdio streams based on config.
                </div>
              </div>
            )}`;

const cliMappingsTabContent = `
            {/* CLI Mappings Panel */}
...
content = content.replace(mcpServerTabEnd, mcpServerTabEnd + '\n' + cliMappingsTabContent);

fs.writeFileSync('src/components/SettingsModal.tsx', content, 'utf8');
console.log('Patched SettingsModal.tsx successfully');
